...
/** The plan's BOUNDING-BOX quad at height `y` — not the plan polygon. There is
  ...
  *
  * On an L-shaped plan the floor is still a plain rectangle covering the cut-out
  * too, and that is fine: every ring edge carries an opaque full-height wall, so
  * the region outside the plan is never visible and the camera cannot reach it.
  * The grime line and the noise fade come from [[edgeSetDist]] against the ring
  * edges, not from the mesh, so they follow the true plan boundary regardless of
  * how far the quad extends past it.
  *
  * `margin` widens it on every side — for a light plane that has to overhang far
  * enough that no reachable sightline finds its edge.
  *
