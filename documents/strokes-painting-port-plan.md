...
- [trivalibs/documents/line2d-plan.md](../trivalibs/documents/line2d-plan.md) —
...
([line2d-plan.md](../trivalibs/documents/line2d-plan.md)) but was never
implemented.
...
Full detail in [line2d-plan.md](../trivalibs/documents/line2d-plan.md) §2.
...
- **New** `trivalibs/src/graphics/geometry/line2d.scala` — `LineVertex[T]`,
  ...
  `splitAtAngle`, `toBufferedGeometry(props)` and
  `Arr[Line[T]].toBufferedGeometries(props)`.
...
**Gate:** `bun run check` + `bun run test` in `trivalibs/`.
...
[line2d-plan.md §3](../trivalibs/documents/line2d-plan.md).

**Geometry** (CPU, rebuilt on resize):

1. `Line[Unit](20.0)`, ~20 random points over 1.5× the canvas, random widths in
   `[20, 300]`.
...
`vec4(uv, 1, 1)` — the old test's uv debug color, which makes mitre and uv
errors visually obvious.

`onResize` regenerates geometry and updates `size`; white clear color.

**Gate:** clean mitre joins, no gaps at the split corners, uv gradient
...
What blocks a direct merge: those shaders and strategies are built around
**four rectangular walls**. The wall texture compositing, shadow rects, and
reflection all assume that geometry, and none of it translates to a hexagonal
room as-is. Working that out is its own design problem.
...
Keeping the painting's output a plain panel texture is what keeps that door
open — no coupling to how it is eventually displayed.
