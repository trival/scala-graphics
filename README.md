...
### Submodule URL: HTTPS fetch, SSH push
...
After that, commit and push inside `trivalibs/` exactly as before. Verify with
...
one-time approval. In the terminal CLI that's an interactive prompt on first
run; in the **VS Code extension that prompt is not surfaced**, so the server is
...
Claude session keep an editor with Metals open in the background (VS Code, or
any Metals client) so the server stays up and `.mcp.json` is current — the CLI
reads `.mcp.json` from the working directory. With no Metals running there is no
